...
public:
  ...
  BOOL SetTooltipText(const CString& Tip){
    if(!OK) return FALSE;
    NID.uFlags=NIF_TIP;
    _tcsncpy_s(NID.szTip, sizeof(NID.szTip)/sizeof(CHAR), Tip, 64);
    NID.szTip[min(64,Tip.GetLength())-1]=0;
...
protected:
  ...
  BOOL Create(LPCTSTR szTip, HICON hIcon, UINT Menu=0) { //Menu may be used by derived classes
    ...
    // Create this invisible window for Message passing
      ...
      _tcscpy_s(NID.szTip, sizeof(NID.szTip)/sizeof(CHAR), szTip);
...
public:
  ...
  virtual ~CAniIcon() {try{delete[] Icons;}catch(...){}}
...
public:
  ...
  virtual ~CAniTrayIcon() {try{StopAnimating();}catch(...){}}
...
public:
  ...
  virtual ~CAniMenuTrayIcon() {try{StopAnimating();}catch(...){}}
